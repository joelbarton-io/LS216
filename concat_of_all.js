...
const findTheSubstring = (s, words) => {
  ...
  let startIdx = 0;
  const START_INDICES = [];
  const STRING_INITIAL = words.join('');
  let wordsAsString = STRING_INITIAL;
  let end = LENG;
  let start = 0;
  ...
  while (end <= s.length) {
    ...
    if (wordsAsString.length === STRING_INITIAL.length) {
      startIdx = start;

      if (wordsAsString.includes(curr)) {
        wordsAsString = wordsAsString.replace(curr, '');

      } else if (!wordsAsString.includes(curr) && wordsAsString.length > 0) {
