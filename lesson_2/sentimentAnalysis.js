...
function sentiment(text) {
  ...
  wordList.forEach(word => {
    if (wordMatches(word, positiveWords)) {
      pos.push(word);
    }

    if (wordMatches(word, negativeWords)) {
      neg.push(word);
    }
  });

  return resultMessage(pos, neg);

  function resultMessage(positives, negatives) {
...
function wordMatches(word, sentiments) {
  return sentiments.every(candidate => {
